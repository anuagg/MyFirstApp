	...
	private class CallAPI extends AsyncTask<String, String, List<objectsmodel>> {
			...
			protected List<objectsmodel> doInBackground(String... params) {
				...
				try {
					...
					JSONArray parentarray = parentobject.getJSONArray("objects");


					for (int i = 0; i < parentarray.length(); i++) {
						JSONObject finalobject = parentarray.getJSONObject(i);
						objectsmodel objectmodel = new objectsmodel();
						objectmodel.setName(finalobject.getString("name"));
						objectmodel.setLocality(finalobject.getString("locality"));
						objectmodel.setStreet_address(finalobject.getString("street_address"));
						objectmodel.setRegion(finalobject.getString("region"));
						objectmodel.setPostal_code(finalobject.getString("postal_code"));
						objectmodel.setCountry(finalobject.getString("country"));
						objectmodel.setPhone(finalobject.getString("phone"));
						objectmodel.setHas_menu(finalobject.getBoolean("has_menu"));
						objectmodel.setLongi(finalobject.getDouble("long"));
						objectmodel.setLat(finalobject.getDouble("lat"));
						objectmodel.setLocu_id(finalobject.getString("id"));
						objectmodel.setWebsite_url(finalobject.getString("website_url"));
